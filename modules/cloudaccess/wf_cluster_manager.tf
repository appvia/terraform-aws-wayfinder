...
resource "aws_iam_role_policy_attachment" "cluster_manager_from_aws" {
  count = var.enable_cluster_manager && var.from_aws ? 1 : 0

  role       = module.iam_role_cluster_manager[0].iam_role_arn
  policy_arn = module.iam_policy_cluster_manager[0].arn

  depends_on = [module.iam_role_cluster_manager[0].iam_role_arn, module.iam_policy_cluster_manager[0].arn]
}
...
resource "aws_iam_role_policy_attachment" "cluster_manager_from_azure" {
  count = var.enable_cluster_manager && var.from_azure ? 1 : 0

  role       = module.iam_role_cluster_manager_azure_oidc[0].iam_role_arn
  policy_arn = module.iam_policy_cluster_manager[0].arn

  depends_on = [module.iam_role_cluster_manager_azure_oidc[0].iam_role_arn, module.iam_policy_cluster_manager[0].arn]
}
...
resource "aws_iam_role_policy_attachment" "cluster_manager_from_gcp" {
  count = var.enable_cluster_manager && var.from_gcp ? 1 : 0

  role       = module.iam_role_cluster_manager_google_oidc[0].arn
  policy_arn = module.iam_policy_cluster_manager[0].arn

  depends_on = [module.iam_role_cluster_manager_google_oidc[0].arn, module.iam_policy_cluster_manager[0].arn]
}
